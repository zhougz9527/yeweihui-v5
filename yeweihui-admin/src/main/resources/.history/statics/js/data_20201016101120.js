        ...
        pickerOptions: {
            shortcuts: [
                ...
                onClick(picker) {
					...
					// 上月初
					const firstTime = new Date(nwoYear,lastMonth)
					// 本月初换算上月末
					if (nwoMonth == 11){
						// 12月,第二年初
						nwoMonth = 0
					}
					const lastTime = new Date(nwoYear,nwoMonth)
					const start = firstTime
					const end
					end.setTime(lastTime.getTime()-3600*1000*24)
                    picker.$emit('pick', [start, firstTime]);
                }
			}
			// , {
            //     text: '一月',
            ...
            //         const end = new Date();
			// 		let first = start.getFullYear()
			// 		let endTime = new Date(first,0)
			// 		let start = new Date(first-1,0)
			// 		end.setTime(endTime.getTime()- 3600*1000*24)
            //         picker.$emit('pick', [start, end]);
            //     }
			// }
