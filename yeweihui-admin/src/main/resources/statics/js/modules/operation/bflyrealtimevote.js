...
var vm = new Vue({
    ...
    methods: {
            ...
            $.ajax({
                ...
                success: function (result) {
                    if (result.code == 0) {
                        ...
                        $("#jqGrid").jqGrid({
                            url: baseURL + 'bflyVote/selectCurVoteList',
                            mtype: "POST",
                            postData:{
                                "page":1,
                                "size":10,
        ...
        reloadVoteData:function(){
            vm.getInfo(vm.curVoteId)
        },
        getInfo: function (id) {
            let _this  = this;

            $.get(baseURL + "bflyVote/curInfo/" + id, function (r) {
        ...
        btnFormatter: function (cellvalue, options, rowdata) {
            var result = "";
            result += '<a class="btn btn-primary btn-sm" target="_blank" onclick="onView(' + rowdata.id + ',1)">投票实时情况查看</a>';
            if(rowdata.isShow===0){
