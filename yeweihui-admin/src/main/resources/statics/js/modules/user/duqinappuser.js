$(function () {
    $("#jqGrid").jqGrid({
        url: baseURL + 'user/duqinappuser/list',
        datatype: "json",
        colModel: [			
			{ label: 'id', name: 'id', index: 'id', width: 50, key: true },
			{ label: '登陆账号', name: 'account', index: 'account', width: 80 }, 			
			{ label: '密码', name: 'password', index: 'password', width: 80 }, 			
			{ label: '用户类型（游客 GUEST、普通用户 NORMAL_USER、读亲达人 DQ_EXPERT、读亲歌手 DQ_SINGER、读亲号 DQ_OFFICIAL_ACCOUNT、管理员 ADMIN） ', name: 'type', index: 'type', width: 80 }, 			
...
var vm = new Vue({
	...
	methods: {
		...
		saveOrUpdate: function (event) {
			var url = vm.duqinAppUser.id == null ? "user/duqinappuser/save" : "user/duqinappuser/update";
			$.ajax({
				type: "POST",
			    url: baseURL + url,
                contentType: "application/json",
			    data: JSON.stringify(vm.duqinAppUser),
			    success: function(r){
			    	if(r.code === 0){
		...
		del: function (event) {
			...
			confirm('确定要删除选中的记录？', function(){
				$.ajax({
					type: "POST",
				    url: baseURL + "user/duqinappuser/delete",
                    contentType: "application/json",
		...
		getInfo: function(id){
			$.get(baseURL + "user/duqinappuser/info/"+id, function(r){
                vm.duqinAppUser = r.duqinAppUser;
            });
		},
