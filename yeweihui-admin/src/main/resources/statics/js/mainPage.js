...
var vm = new Vue({
    ...
    computed: {
        weaImgUrl() {
            if (this.wData && this.wData.wea_img) {
                return "https://xuesax.com/tianqiapi/skin/pitaya/" + this.wData.wea_img + ".png";
            } else {
                return null;
            }
        }
    ...
    methods: {
        ...
        getWeather() {
            $.get("https://www.tianqiapi.com/api?version=v6&appid=48136697&appsecret=sU3fDo0n", function (r) {
                vm.wData = r;
                // console.log(r);
            });
        },
        ...
        getUserList() {
            ...
            $.ajax({
                type: "GET",
                url: baseURL + 'sys/user/list?' + `page=${self.currentPage}&limit=${self.pageSize}&order=asc`,
                contentType: "application/json",
                success: function (r) {
                    if (r.code === 0) {
                        self.userList = r.page.list;
