    ...
    Video.prototype = {
        ...
        _createInsertPanel: function _createInsertPanel() {
            ...
            var tabsConfig = [{
                ...
                // 模板
                // tpl: '<div>\n                        <input id="' + textValId + '" type="text" class="block" placeholder="\u683C\u5F0F\u5982\uFF1A<iframe src=... ></iframe>"/>\n                        <div class="w-e-button-container">\n                            <button id="' + btnId + '" class="right">\u63D2\u5165</button>\n                        </div>\n                    </div>',
                tpl: '<div>\n                        <input id="' + textValId + '" type="text" class="block" placeholder="直接插入网址"/>\n                        <div class="w-e-button-container">\n                            <button id="' + btnId + '" class="right">\u63D2\u5165</button>\n                        </div>\n                    </div>',
                // 事件绑定
                events: [{
                    selector: '#' + btnId,
                    type: 'click',
                    fn: function fn() {
                        ...
                        if (val) {
                            // 插入视频
                            // _this._insert(val);
                            _this._insert("<iframe src='"+ val +"' frameborder=0 allowfullscreen></iframe>");//height=498 width=510 max-width: 50%;max-height:50%; height:100%;
                            // _this._insert('<video src="' + val + '" style="max-width: 50%;max-height:50%;" controls autobuffer><video/><p><br></p>');
                        }
    ...
    UploadVideo.prototype = {
        ...
        uploadVideo: function uploadVideo(files) {
            ...
            var client = new OSS.Wrapper({
                region : 'oss-cn-beijing',//oss-cn-beijing.aliyuncs.com
            ...
            // 在key后面加时间戳
            var keyBefore = file.name;
            var keyArr = keyBefore.split('.');
            keyArr[0] = keyArr[0]+'_'+new Date().getTime();
            var key = keyArr.join(".");

            var loding = parent.layer.msg('上传中,请稍等', { icon: 16 ,shade:0.5,time:0 });
            client.multipartUpload(key, file).then(function (result) {
                ...
                // editor.cmd.do('insertHTML', '<p><video src="' + returnUrl + '" style="max-width: 50%;max-height:50%;" controls autobuffer><video/><br><p/>');
                // editor.cmd.do('insertHTML', '<img src="' + link + '" style="max-width:100%;"/>');
                editor.cmd.do('insertHTML', '<video src="' + returnUrl + '" style="max-width: 50%;max-height:50%;" controls autobuffer><video/>');//<p><br></p>
                // editor.txt.append('<p><video src="' + returnUrl + '" style="max-width: 50%;max-height:50%;" controls autobuffer><video/><br><p/>');
    ...
    Editor.prototype = {
        ...
        _initDom: function _initDom() {
            ...
            if (textSelector == null) {
                ...
                $textContainerElem.css('border', '1px solid #ccc').css('border-top', 'none').css('height', '550px');/*cutie modify*/
            } else {
